...
*A tick puts work down*). A wave never waits on its slowest agent to merge its fastest. This file governs that machinery:
...
- Resolve the worktree-local mirror of the declared channel — the state root's repo-relative
  ...
  it **that is untracked at the worktree's HEAD** into the primary `<flumeDir>/<friction>/`,
  prefixing each filename with `<tag>--` for provenance and to separate sibling entries in the
  same wave from each other and from the primary's own revert notes.
- **The tracked-at-HEAD bound is what keeps the relay convergent, and what keeps it from
  duplicating a delivery.** A note an agent *commits* (against convention, but nothing
  prevents it) becomes tracked content every subsequent worktree checks out; a relay that
  moves whatever it finds re-deposits that note once per wave member, unbounded —
  field-paid downstream: one committed note became eight stamped copies across two waves,
  plus six more per wave until an operator intervened. A file the checkout itself brought
  in is delivered history, not mail. The same bound covers the note *this* tick committed:
  its commit is the delivery, and the ship or the revert snapshot (*Reverted prose survives
  the reset*) carries it, so a second harvested copy under a stamped name is a duplicate the
  operator would have to reconcile by hand. The relay carries only what this tick's agent
  produced and did not commit.
