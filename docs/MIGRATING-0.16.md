...
`0.17.0`). Its breaks reach the API, the command line, and the on-disk format
of the two files that guard a running loop — read it whole, and read it
before a fleet runs two versions against one state root.
