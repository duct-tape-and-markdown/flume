...
1. **Refuse if branch `<name>` already exists** — no clobber; the clean branch is named by the job, and a pre-existing branch of that name is someone's work (exit `1`). Also refused up front: uncommitted tracked changes (extract rewrites the checkout — fork, cherry-pick, possibly unwind), and a live job loop (as with `rm`; a stale pidfile is reclaimed silently).
...
4. **Harvest `friction.md` and `plan/open-questions.md`** from the job branch tip (`git show`, not the worktree) to stdout for operator routing; a file that never existed is noted instead of printed.
...
Exits `0` on success; `1` on the clobber/dirty-tree/live-loop refusals, a git failure, or an unwound cherry-pick conflict; `2` on usage errors (missing `<name>` or `--onto`, a flag without a value, a `<name>` that is not a single segment, an `--onto` that resolves to no commit, or a `<name>` with no `job/<name>` branch to extract).
