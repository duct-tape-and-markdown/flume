...
non-numeric, or given any other unrecognized argument; `74` (`EX_IOERR`) when
`tick-verdicts.jsonl` is present but cannot be read — exit `0` over silence
means the log is not there, never that it could not be opened.
