...
| plan  | `.flume/plan/{pending.json,state.json,questions/}`; drains `.flume/inbox/`, `.flume/plan/notes/` | plan tick | `plan:` | `plan:` |
...
- **pending.json is derived, not authored.** Plan re-derives it every tick from spec + questions + inbox + current src. Never hand-edit. Cross-tick context belongs in `.flume/plan/questions/`.
- **Open questions go in `.flume/plan/questions/`, one file each, not in pending.json.** If a candidate plan entry can't carry a clean `per` cite into the spec, it's a question for a human.
