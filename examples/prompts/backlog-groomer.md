...
Read `{{BACKLOG_PATH}}` at the repo root — a JSON array of items shaped like:
...
2. Remove it from `{{BACKLOG_PATH}}`.
3. Append one line to `{{SHIPPED_PATH}}`: `- <tag>: <reason>`.
...
5. Commit `{{BACKLOG_PATH}}` and `{{SHIPPED_PATH}}` together, one commit,
   prefixed `groom:`.
