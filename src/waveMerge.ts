 ...
 * per-entry revert when one turns red, the `shipped` consult — and then, once
 * the wave's last attempt has been carried, the pending-ledger rewrite
 * (`src/pendingLedger.ts`) that retires what shipped, answering the merge
 ...
 * {@link closeWaveMerge} writes the ledger. Each of the latter two is its own
 * ship-lock span — taken when it starts touching trunk and released however
 * it leaves, shipped, reverted or thrown as a {@link WaveLedgerRefusal} — so
 * a sibling tick gets its turn between two of this wave's picks. What one
 * span learned and the next needs is {@link WaveMerge}.
 *
 ...
 * {@link closeWaveMerge}. By this point the wave's cherry-picks and afterMerge
 * gates already landed `shippedTags` on trunk, so the verdict recording them must
 ...
 * teardown*), so what one pick learned — the markers it staked, the
 * bystander checkpoint it took, the outcomes and gate rows it recorded — has
 * to outlive that pick and reach the ledger write the wave ends with. Every
 * field below is an accumulator, and nothing reads across entries except the
 * three the wave is explicitly once-per-wave about: the checkpoint, the
 * marker set, and the ledger write itself.
 */
interface WaveMerge {
   ...
   * gate rows {@link closeWaveMerge} reports.
   */
   ...
   * marker for, retired together once the ledger rewrite lands.
   */
  readonly mergingSlugs: Set<string>;
...
export async function mergeAttempt(
  ...
  // The cherry-pick, the `afterMerge` judges that read the merged tip and the
  // revert that may follow all touch trunk — so this entry's whole passage is
  ...
  try {
    ...
    w.mergingSlugs.add(slugify(r.entry.tag));
...
/**
 * Close the stage once every attempt has been carried: the pending-ledger
 * rewrite that retires what shipped and records what each entry touched
 * (`src/pendingLedger.ts`), the merge markers it retires, and the facts the
 * wave leg folds into its `TickResult` and verdict.
 *
 * Under the ship lock for the same reason each pick is: the rewrite is a
 * commit on trunk.
 *
 * Throws {@link WaveLedgerRefusal} when that rewrite refuses: by then the
 * picks have already landed, so the facts this would have returned ride the
 * error as a verdict instead of vanishing with it.
 */
export async function closeWaveMerge(w: WaveMerge): Promise<WaveMergeResult> {
  const {
    leg,
    phase,
    provisioned,
  ...
  const allGateResults = [...w.attemptGateResults, ...w.mergeGateResults];
  const committedWave = w.shipped.length > 0;
  ...
  try {
    ...
    const footprintTags = w.mergeOutcomes.flatMap((m) =>
      m.entryTag && m.footprint && m.footprint.length > 0 ? [m.entryTag] : [],
    );
    if (w.shipped.length > 0 || footprintTags.length > 0) {
      ...
      try {
        ...
        const noCommit = waveNoCommitCause(committedWave, w.attempts, [
          ...w.mergeReverted,
          ...w.revertRefused,
        ]);
    ...
    await clearMergingMarkers(leg, w.mergingSlugs);
  } finally {
    // Every way out of the span: rewritten, no-op'd, or thrown past its own
    // bookkeeping by `WaveLedgerRefusal`.
    shipLock.release();
  }
