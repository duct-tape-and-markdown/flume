 ...
 *
 * Its own module because three surfaces ask the same question and none of
 ...
 * that put its work down; and the records gate, which holds a *finishing*
 * commit to taking the entry's continuation with it. A copy at any of them
...
export function putDownPredicate(stateRoot: string): PutDownPredicate {
  return (entry, span) => {
    if (span.touched.includes(parkedNotePath(stateRoot, entry.tag)))
      return "parked";
    const continuing = continuingNotePath(stateRoot, entry.tag);
    if (
      span.touched.includes(continuing) &&
      existsLoud(namespacedJoin(span.tree, continuing))
    )
      return "continuing";
    return undefined;
  };
