...
Drain the records. Each file in `<records>` is one record: a finding someone left in the inbox queue, or a note a build tick left under the notes queue. **A note's directory is its kind** — one under `notes/parked/` is a park (the entry could not ship as written and stayed in the queue; widen its `files`, split it, or answer what it parked), and one beside that directory is an observation from a tick that shipped. Read the path, never the prose, for which it is. Route every record, then `git rm` its file. The record queues are queues, not logs; never create a record yourself — the records gate refuses a plan commit that does.
