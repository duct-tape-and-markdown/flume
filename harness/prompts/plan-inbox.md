...
**A family noted three times files once.** A shape accepted as debt in three of this slice's own `plan:` commit bodies has cost more in re-noting than filing it would. File it then: one entry that claims no property — no `tests[]`, no `pins[]` — names the target shape, names every site in `files`, and ships on its own. Read the count off `git log` over those bodies, never estimated; under three, shape stays out of the queue.
