...
Not injected into any tick: each slice's prompt names this file's path, and you open it once before writing the queue, the plan state artifact, or a question. One home for the rules the slices share; `PROTOCOL.md` holds what an entry carries and what makes one good. Every path this page speaks of by name is spelled out in the `<artifacts>` block of the slice prompt that sent you here.
...
- **The plan state artifact** — four typed fields: `derivedThrough` (the sha the spec has been derived through), `sweptThrough` (the sha the posture frontier was derived from), `rotation` (`closed`, or `open` carrying the covered set the sweep has settled), and `drainedRuns` (per declared CI lane, the run that lane was last drained at and the failing titles that run stated). A slice edits its own field and leaves every other one byte-identical — with one field two slices write: `derivedThrough` is the derive slice's, and the inbox drain's through a spec commit whose derivation it routed from a record. A cursor advances only to a sha the slice actually processed through, and only to one the slice's own rendered window named — never as bookkeeping, never reflexively to HEAD, never one the slice resolved for itself — and is carried forward verbatim otherwise. The artifact is refused outright if a cursor or the rotation is missing, so there is no "no cursor" state to fall back into: a slice with nothing to advance writes back the cursor it came in with. `drainedRuns` is the one field absence is a state in: a lane missing from the map — and a whole artifact carrying no map — reads as that lane never drained, so stamping your first lane is an added field, not a repair. A stamp is `{ "run": "<id>", "titles": [...] }` and the window hands you the exact value to write; a bare run identity left by an older tick reads as that run with no titles. A sha that does not resolve makes that slice's window refuse, and the repair goes in the commit body.
