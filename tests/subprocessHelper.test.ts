 ...
 *
 * One job, read two ways: the wrappers' own cases, and the scans that keep
 * this helper the only spelling of a captured spawn under `tests/` — a
 * wrapper is a cap only while it is the single one, so the scans are the
 * cases' own reach rather than a second subject.
 */
