...
## The `records` gate judges one commit of a possibly multi-commit span (PARKED — `.flume/chain.ts` is outside every phase lane)

Posture sweep, neighborhood `tests/chain.test.ts` + `.flume/chain.ts`; verified on
disk this tick. `recordsGate` builds its own touched list by shelling `git
diff-tree --no-commit-id --name-status -r --root <commitSha>`
(`.flume/chain.ts:265`) — the **single commit**'s diff. Every other gate in this
chain reads `ctx.touchedPaths`, which the dispatcher computes once per commit as
the whole `spanBase..commitSha` diff and shares (`src/Dispatcher.ts:3586`,
`git.diffNameOnly`), because a multi-commit span is one completed tick
(`spec/loop.md`, *N commits are completion*).

**What it costs.** A build tick that commits a sibling's note — or its own,
untitled or over the 1200-byte cap — and then makes a second commit carrying code
sees `records` report `no records touched` and pass, while `writable-paths`
happily fences the whole span (`${NOTES_DIR}/*.md` is inside build's fence). The
gate's entire claim — one record, own tag, titled, capped — is void over any span
whose *last* commit happens to carry no record.

Section: `.claude/rules/engineering.md`, *A fact the engine holds is reported,
never rediscovered* — "a chain restating an engine fact is evidence against the
engine, not the chain."

**The fork.**

1. **Chain-only.** `recordsGate` reads `ctx.touchedPaths` and derives
   written-vs-drained from `api.git.readFileAtRef(repoRoot, commitSha, path) ===
   null` — the call it already makes for every written record. A tree-to-tree span
   diff never lists a path created and removed inside the span, so the status is
   exact, not approximate. One `chore(flume):`; the `execFileSync` and the
   `--name-status` column parsing both go.
2. **Engine widens.** `GateContext` gains per-path change status beside
   `touchedPaths` (the same `git diff`, `--name-status`), and the chain adopts. A
   build entry plus a `chore(flume):`.

**Recommend the first.** The engine already reports both halves the gate needs;
option 2 buys a new engine surface for one consumer with no second asker
(`engine-boundary.md`, *Surface, not prescription*). Parked rather than filed
only because `.flume/chain.ts` is outside every phase lane.

**The suite is blind to it either way.** Every case under *records gate and the
park predicate* (`tests/chain.test.ts:618`) hand-builds a `GateContext` with no
`touchedPaths` over a single-commit fixture. Whichever option lands carries a
two-commit-span case — a record in the first commit, code in the second — as its
`tests[]` line.
