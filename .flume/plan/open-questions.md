...
## FRICTION-DECLARATION entry omitted its own test file from `files` — second live instance of the fence-mismatch above (NEEDS AMENDMENT)

**Status:** NEEDS AMENDMENT — this tick shipped `FRICTION-DECLARATION`
(`Chain.friction?: string` + load-time validation) touching only
`src/Phase.ts` + `src/Dispatcher.ts`, exactly as `entry.files` declared.
It deliberately did **not** touch `tests/Dispatcher.test.ts`, even though
`entry.tests` names that path with concrete `asserts`, because the prior
attempt on this same entry (`231440e`, reverted) proved
`entry.tests[].path` does not enter the write allowance —
`writablePathsGate`'s entry-scoped fence is `entry.files.{new,edit,retire}
∪ phase.entryChannelPaths` only (`src/Dispatcher.ts:1056-1068`); tests
never make that union unless separately listed under `files.new`.

**Consequence:** `FRICTION-DECLARATION`'s acceptance
("chain load rejects an absolute or outside-state-root friction path;
accepts a valid relative one") shipped with manual verification only — no
committed test asserts it. `tests/Dispatcher.test.ts` still has zero
coverage for `Chain.friction` validation.

**Fix shape:** when plan derives an entry whose `tests[].path` is an
existing file (edit, not new), that path belongs in `files.edit` too —
`entry.tests` should describe *what* to assert, not silently grant *where*
to write it. This is the same root cause as the harness-block question
above (the engine narrows the fence to `files ∪ entryChannelPaths` but
plan's own entry-authoring surface doesn't cross-check `tests[].path`
against `files` before emitting pending.json), just caught from the build
side this time with a concrete before/after: attempt 1 wrote the test and
was reverted; attempt 2 (this tick) skipped it and shipped clean but
uncovered.

**Recommendation:** re-derive `FRICTION-DECLARATION`'s test coverage as a
follow-up entry with `tests/Dispatcher.test.ts` listed under `files.edit`
explicitly (not just `tests[]`), scoped narrowly to the friction
validation asserts already specified. Longer-term: plan's entry-derivation
step should assert `tests[].path ⊆ files.new ∪ files.edit` before writing
pending.json, closing this class for every future entry rather than
one-off.
