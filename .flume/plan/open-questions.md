...
## The sweep's absence-verdict tool is not on this host, so consumerless-export findings cannot be made (PARKED — rule amendment or a dependency)

`.claude/rules/engineering.md` *An export earns its consumer* closes with a
hard procedural bar: "An absence verdict never rests on a bare text search.
Confirm a zero-consumer or dead-symbol finding with LSP references
(`code-navigation.md`), never a plain no-hits." `posture-sweep.md` repeats it
as a standing lens.

**The tool is absent.** `typescript-language-server` is not on PATH in the
sweep worktree — `LSP findReferences` returns ENOENT. `code-navigation.md`
says the plugin "requires `typescript-language-server` on PATH (globally
installed)", so the bar depends on a host fact no repo artifact holds and no
gate checks.

**It has now cost three sweep ticks in this rotation**, read off `git log`
from the cursor rather than estimated:

- `7014b71` — `defaultLaneFiles` (`tests/helpers/spawnBudget.ts:72`): "the
  finding is left unmade rather than filed on a bare no-hits."
- `7f5f960` — `scripts/build-changelog.mjs`'s `resolveLastRelease`,
  `deriveEntries`, `renderSection`, plus an `isDirectInvocation` doc block
  justifying itself by a consumer that does not exist: same caveat verbatim.
- this tick — `WAIT_TIMEOUT_MS` / `WAIT_INTERVAL_MS`
  (`tests/helpers/waitFor.ts:30,33`): repo-wide identifier search finds no
  reader outside the module and no namespace import of the module, but that
  is a text search.

*Routing*'s three-strikes clause does not discharge this. That clause files a
recurring shape **once**, as an entry naming every site — but it presupposes
the findings were *made*. These were not. What recurs is an unmakeable
verdict, not an accepted defect.

**The fork, which is yours** — all three change a rule page or a manifest,
neither of which build may touch:

1. **Make the tool a repo fact.** Add the language server to
   `devDependencies` so the bar is satisfiable from a clean clone.
   Honest to the page as written; costs a dev dependency whose only consumer
   is agent navigation, and nothing would gate its presence.
2. **Let the repo answer it instead.** `typescript` is already a
   `devDependency`, and the suite already walks TS ASTs with it
   (`tests/helpers/spawnBudget.ts`, `tests/Gate.test.ts`) — a symbol-reference
   scan is buildable in-tree today with no new dependency, and would be a
   *pin* rather than a per-tick agent action. The ecosystem's off-the-shelf
   answers for this shape are `knip` / `ts-prune`; neither has been trialled
   here. This is the option I would take, but it **contradicts the page as
   written**, which names LSP specifically — so the page moves first.
3. **Relax the bar.** State what confirmation short of LSP counts (an
   identifier search over tracked files plus a namespace-import rule-out, say).
   Cheapest; weakest.

Until it is ruled, every consumerless-export finding routes to a debt line
with the caveat attached, which is the cost this question is asking you to
price.
