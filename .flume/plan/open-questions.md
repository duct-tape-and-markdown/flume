...
The prompt half of that note ships as
`EXAMPLE-PLAN-PROMPT-SPANS-READ-THE-INJECTED-ROOT`; this is the half it cannot
reach.

**The corpus contradicts itself.** `spec/prompt.md` *The reserved
`{{FLUME_DIR}}` prompt arg* says `writablePaths` "stays derived from
`process.env.FLUME_DIR` at chain-load because it is static config evaluated
before any per-tick context exists". `spec/chain.md` *Per-run artifacts belong
under `FLUME_DIR`* says the chain "never reads `process.env.FLUME_DIR`" and is
handed `api.paths` instead. `docs/CHAIN-AUTHORING.md` takes the second side and
names `writablePaths` explicitly ("chain-load (placement, `writablePaths`) →
`flume.paths.flumeDir`"), then warns that hardcoding `.flume/` in a fence
"breaks under a relocated `flumeDir`".

**Neither side is expressible today.** `writablePaths` entries are
repo-relative git pathspecs; `api.paths.flumeDir` is absolute. The offset
between them is `computeStateRootRel`, which `src/index.ts` does not export —
`harness/chain.ts` reaches it through a relative `../src/Dispatcher.js` import
(line 42, consumed by `repoRelativeStateRoot` at 641) that no published
consumer can make. So a downstream chain's only options are
the hardcoded literal or a hand-rolled `relative()`, which is the engine fact
being rediscovered (`.claude/rules/engineering.md`, *A fact the engine holds is
reported, never rediscovered*). `examples/cascade-chain.ts:588-594` takes the
literal — the doc's own footgun, in the file the doc quotes.

**The fork:**

1. **Report the offset.** `stateRootRel` on `FlumePaths` (or
   `computeStateRootRel` on the export list), the `process.env` clause deleted
   from `spec/prompt.md` in the same edit, and cascade derives its fence.
   Recommended: it is the rule the doc already states, and the engine already
   computes the value per tick for `GateContext`/`TickContext`.
2. **Ratify the env read for fences only.** Then `spec/chain.md`'s absolute
   "never reads `process.env.FLUME_DIR`" and the doc's boundary table both take
   an edit, and a chain-load env read becomes sanctioned in exactly one place.
3. **Rule consumer fences out of relocation entirely** — the harness package
   already refuses a relocated root (`spec/harness.md`, *Committed-path
   discipline*). Then the doc's `writablePaths` clause goes and cascade's
   literal is correct as written. Cheapest; costs the `--job` story for
   downstream chains.

`spec/**` is yours either way, so nothing here can be an entry until the ruling
lands. Under (1) the follow-on is a `src/` entry plus a cascade edit, both
