...
- **Build notes** — `.flume/plan/notes/<TAG>.md` for an observation the next plan tick should know, `.flume/plan/notes/parked/<TAG>.md` for a **park**: the entry cannot ship as written — its premise is contradicted by the tree, it needs a decision nobody made, or the work is already there — and `build.shipped` keeps it in the queue. The park is the file's location, never the shape of the commit that wrote it. Written by the build tick assigned that entry and no other; first line `# <title>`. A park is never about paths: there is no per-entry fence to fall outside.
- **Open questions** — `.flume/plan/questions/<slug>.md`, one file each, present while open and deleted by the drain that answers it. Any session may add one; the status is that it exists (`spec/harness.md`, *Records as one file each*).

**A record is short.** It says what was observed, where (a path and line, or a sha), and why it matters — at most **2,000 bytes**, measured as bytes (`wc -c`), not characters: an em-dash is three. Options appear only when a decision genuinely forks, one line each. What a record does not carry: the reasoning that led to it, restated spec, or a proposed patch; the reader re-verifies against the tree regardless. The `records` gate refuses a commit that writes a record without a title line, under a tag that is not the tick's own, or from a plan slice — plan **drains** records and never creates one. A record over the cap is not refused: it ships with its entry, and the drain names the overrun in the plan commit body.
