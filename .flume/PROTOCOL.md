...
- **Build notes** — one file per entry under `.flume/plan/notes/`, written by the build tick assigned that entry and no other; first line `# <title>`. Which directory a note lives in is its kind — an observation, a park, a continuation — and the homes are the harness package's, named in its build prompt and in `spec/harness.md`, *Records as one file each* and *A tick puts work down*; this page does not restate them. A park is never about paths: there is no per-entry fence to fall outside.
